...
class Config:
    ...
    # Run compute_anchors(train_loader.samples) once to optimise for your data.
...
# =============================================================================
# COCO DATA LOADER  (matches leaves COCODataLoader pattern)
# =============================================================================
...
class COCODataLoader:
    ...
    Matches the leaves COCODataLoader API: accepts category_mapping kwarg
    so val/test splits share the same category index as train.
    """
    ...
    def __init__(self, data_dir: Path, split: str = "train",
        ...
        # Images may live in a sub-folder or directly in the split folder
    ...
    def _build_category_mapping(self):
        ...
        # Find avocado category (case-insensitive); fall back to first category
        self.cat_id = None
        for cat in cats:
        ...
        # For a detector we only care about one class, so mapping is trivial
        self.category_id_mapping = {cat["id"]: 0 for cat in cats}

    def get_category_mapping(self) -> dict:
        """Return mapping dict for sharing with val/test loaders."""
        return {
    ...
    def _parse(self) -> list:
        ...
        for img_id, anns in ann_map.items():
            ...
            boxes = []
            for ann in anns:
                bx, by, bw, bh = ann["bbox"]   # COCO: x_tl, y_tl, w, h
                if bw <= 0 or bh <= 0:
                    continue
...
# =============================================================================
# ANCHOR K-MEANS  (run once, paste results into cfg.ANCHORS)
# =============================================================================

def compute_anchors(samples: list, n: int = 3, iters: int = 300) -> list:
    """
    K-means on ground-truth box dimensions to find optimal anchor shapes.
    Units: grid-cell widths/heights.

    Usage before training:
        cfg.ANCHORS = compute_anchors(train_loader.samples)
    """
    wh = np.array(
...
class LabelEncoder:
    def encode(self, boxes: list) -> np.ndarray:
        """
        boxes : list of [x1,y1,x2,y2] in [0,1]
        Returns: np.ndarray (GRID_H, GRID_W, NUM_ANCHORS, BOX_ATTRS)
        """
        t = np.zeros(
        ...
        for x1, y1, x2, y2 in boxes:
            ...
            t[gj, gi, best_a, 4] = 1.0   # objectness
            t[gj, gi, best_a, 5] = 1.0   # class: avocado
        return t


# =============================================================================
# DATA GENERATOR  (handles bounding-box-aware augmentation)
# =============================================================================
...
class AvocadoGenerator(tf.keras.utils.Sequence):
    ...
    def _augment(self, img: np.ndarray, boxes: list):
        """Bounding-box-safe augmentations."""
        # Horizontal flip
...
def build_model() -> Model:
    """
    MobileNetV2 backbone (fine-tune top 30 layers) +
    two-conv YOLO detection head.
    Output shape: (batch, GRID_H, GRID_W, NUM_ANCHORS, BOX_ATTRS)
    """
...
class YOLOLoss(tf.keras.losses.Loss):
    """
    coord_loss — MSE on (tx,ty,tw,th) for responsible anchor cells
    obj_loss   — weighted MSE on objectness (lambda_noobj for empty cells)
    cls_loss   — MSE on class confidence for responsible cells
    """
...
def decode_predictions(grid: np.ndarray,
    ...
    """
    grid       : (GRID_H, GRID_W, NUM_ANCHORS, BOX_ATTRS) — single image
    Returns    : list of {"box": [x1,y1,x2,y2], "score": float}  (normalised)
    """
...
# =============================================================================
# PLOTS & VISUALISATION  (matching leaves pipeline)
# =============================================================================

def plot_dataset_distribution(samples: list, split_name: str):
    """
    Histogram of avocados-per-image for a dataset split.
    Saved to result_avocado/<split_name>_distribution.png
    """
    ...
    bars = ax.bar([str(x) for x in xs], ys, color="#4CAF50", alpha=0.85)
    ax.set_xlabel("Avocados per image", fontsize=12, fontweight="bold")
    ...
    plt.close(fig)
    print(f"  Distribution chart → {out}")


def plot_training_history(history):
    """Dual loss + optional metric plot (matches leaves pattern)."""
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))

    axes[0].plot(history.history["loss"],     label="Train Loss", lw=2, color="#2196F3")
    axes[0].plot(history.history["val_loss"], label="Val Loss",   lw=2, color="#FF5722")
    axes[0].set_title("Training & Validation Loss", fontsize=13, fontweight="bold")
    axes[0].set_xlabel("Epoch"); axes[0].set_ylabel("Loss")
    axes[0].legend(); axes[0].grid(alpha=0.3)

    # Learning rate (if ReduceLROnPlateau is used)
    if "lr" in history.history:
        axes[1].plot(history.history["lr"], lw=2, color="#9C27B0")
...
def visualise_detections(image_path: str, results: list,
    ...
    """
    Draw colour-coded detection boxes (and optional GT boxes) on the image.
    Saves to result_avocado/<filename>.

    Box colours by ripeness (if provided):
      detection only → green | Unripe → dark-green | Nearly Ripe → orange
      Ripe → bright-green   | Overripe → brown
    """
    COLORS = {
        None:          (0,   220,  0),
        "Unripe":      (34,  139, 34),
    ...
    # Draw ground-truth boxes in blue dashes
    if gt_boxes:
        ih, iw = img.shape[:2]
    ...
    plt.close(fig)
    print(f"  Detection image → {out}")
    return str(out)


def save_model_info(history, train_samples: list, valid_samples: list,
                    ap_valid: float, ap_test: float):
    """
    Save training metadata to result_avocado/model_info.json.
    Mirrors leaves save_model_info() pattern.
    """
    ...
    info = {
        ...
        "task":                "avocado detection (single-class)",
    ...
    for k, v in info.items():
        print(f"  {k:<28}: {v}")
    print("=" * 62)
    print(f"  Model info → {out}")


# =============================================================================
# STAGE 1 DETECT  (used by evaluate_map and run_test_inference)
# =============================================================================

def detect_avocados(model: Model, image_path: str,
                     ripeness_model=None) -> list:
    """
    Run Stage 1 detection (+ optional Stage 2 ripeness) on a single image.

    Returns list of:
      {"box": [px1,py1,px2,py2], "score": float, "ripeness": str|None}
    """
...
def evaluate_map(model: Model, samples: list,
    ...
    """
    Compute mAP@0.5 and save a PR curve plot.
    Mirrors leaves evaluate_model() structure.
    """
...
# =============================================================================
# TEST MOSAIC PREVIEW  (matches leaves visualize_predictions pattern)
# =============================================================================

def run_test_inference(model: Model, n_preview: int = 9):
    """
    Detect on test images and save a preview mosaic.
    Matches leaves visualize_predictions() style.
    """
    print(f"\n  Test-set preview ({TEST_DIR}) ...")
    test_loader = COCODataLoader(DATASET_ROOT, "test")
    ...
    fig.suptitle(
        f"Test Set Preview — {len(samples)} images total\n"
        "(green boxes = predictions, blue dashes = ground truth)",
        fontsize=13
    )
    fig.tight_layout()
    out = RESULT_DIR / "test_preview_mosaic.jpg"
    fig.savefig(str(out), dpi=120, bbox_inches="tight")
    plt.close(fig)
    print(f"  Test mosaic → {out}")
...
def inspect_dataset(json_path: Path, split: str = ""):
    """Quick sanity-check matching leaves data-loading prints."""
...
# =============================================================================
# MAIN PIPELINE  (numbered steps matching leaves main())
# =============================================================================
...
def main():
    ...
    for json_path, split in [(TRAIN_JSON,"train"),(VALID_JSON,"valid"),(TEST_JSON,"test")]:
        if json_path.exists():
    ...
    # ── Optionally compute anchors from training data ─────────────────────────
    # print("\n  Computing anchors from training data...")
    # cfg.ANCHORS = compute_anchors(train_samples)
    ...
    # Copy best model to the production directory that ripeness_routes.py reads
    import shutil
    ...
    # Reload best model with custom loss
    ...
    run_test_inference(best_model, n_preview=9)

    # Quick sample visualisation of first test image
    if test_samples:
    ...
    save_model_info(history, train_samples, valid_samples, ap_valid, ap_test)
