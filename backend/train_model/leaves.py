...
BASE_DIR = SCRIPT_DIR.parent / "datasets" / "leaves-v2"
MODEL_DIR = SCRIPT_DIR / "models_leavesv2"
RESULTS_DIR = SCRIPT_DIR / "results_leavesv2"
...
class COCODataLoader:
    """Load and process COCO format dataset with class balancing"""
    
    def __init__(self, data_dir, split='train', category_mapping=None):
        self.data_dir = Path(data_dir) / split
        # Check if images are in a subfolder or directly in the split folder
        images_subdir = self.data_dir / 'images'
        if images_subdir.exists():
            self.images_dir = images_subdir
        else:
        ...
        # FIX: Create proper category mapping without duplicates
        if category_mapping is None:
            # Build category mapping from COCO data
            raw_categories = {}
            for cat in self.coco_data['categories']:
                cat_id = cat['id']
                cat_name = cat['name']
                raw_categories[cat_id] = cat_name
            
            # Create clean mapping: original_id -> new_sequential_id
            unique_names = sorted(set(raw_categories.values()))  # Sort for consistency
            ...
            # Map original COCO category IDs to new sequential indices
            self.category_id_mapping = {
                orig_id: self.name_to_idx[name] 
                for orig_id, name in raw_categories.items()
            }
            
            self.num_classes = len(unique_names)
            self.categories = self.idx_to_name  # Use clean mapping
        else:
    ...
    def get_dataset(self, balance=False, target_per_class=None):
        ...
        """
        # First, collect all samples organized by class
        samples_by_class = {i: [] for i in range(self.num_classes)}
        
        for img_id, img_info in self.images.items():
            if img_id in self.annotations_by_image:
                try:
                    # Get the category of the first annotation (primary disease)
                    original_category_id = self.annotations_by_image[img_id][0]['category_id']
                    label = self.category_id_mapping[original_category_id]
                    
                    samples_by_class[label].append((img_info, label))
                except Exception as e:
                    print(f"Warning: Skipping image {img_info['file_name']}: {e}")
        ...
        for class_idx in range(self.num_classes):
            for img_info, label in samples_by_class[class_idx]:
                try:
                    img = self.load_image(img_info)
                    images_list.append(img)
                    labels_list.append(label)
                except Exception as e:
                    print(f"Warning: Could not load {img_info['file_name']}: {e}")
...
def main():
    ...
    print("="*60)
    print("LEAF DISEASE DETECTION MODEL TRAINING (BALANCED)")
    print("="*60)
    
    # Load datasets
    print("\n[1/7] Loading datasets...")
    train_loader = COCODataLoader(BASE_DIR, 'train')
    ...
    valid_loader = COCODataLoader(BASE_DIR, 'valid', category_mapping)
    test_loader = COCODataLoader(BASE_DIR, 'test', category_mapping)
    ...
    if len(class_list) != len(set(class_list)):
        ...
        print("This should not happen. Check your COCO annotation file.")
        return
